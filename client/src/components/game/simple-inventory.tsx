    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          ...
          {/* Quick Access Consumable Slots */}
          <div className="mb-4 p-3 bg-green-50 rounded-lg border border-green-200">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-green-800">🍽️ Acesso Rápido</span>
            </div>
            <div className="grid grid-cols-2 gap-2">
              {(() => {
                // Find first food item (restores hunger)
                const foodItem = inventory.find(item => {
                  const itemData = getItemById(item.resourceId);
                ...
                // Find first drink item (restores thirst primarily)
                const drinkItem = inventory.find(item => {
                  const itemData = getItemById(item.resourceId);
                  if (!itemData || !isConsumable(itemData)) return false;
                  const effects = getConsumableEffects(itemData);
                ...
                const foodData = foodItem ? getItemById(foodItem.resourceId) : null;
                const drinkData = drinkItem ? getItemById(drinkItem.resourceId) : null;
                ...
                return [
                  ...
                  <div
                    key={slot.id}
                    onClick={() => {
                      if (slot.item && slot.itemData) {
                        consumeMutation.mutate(slot.item.resourceId);
                      }
                    }}
                    className={`
                      aspect-square border-2 rounded-lg flex flex-col items-center justify-center
                      cursor-pointer transition-all hover:scale-105 p-2
                      ${slot.item ? "bg-white border-green-300 hover:bg-green-50" : "bg-gray-50 border-dashed border-gray-300"}
                    `}
                  >
                    {slot.item && slot.itemData ? (
                      <>
                        <span className="text-xl mb-1">{slot.itemData.emoji}</span>
