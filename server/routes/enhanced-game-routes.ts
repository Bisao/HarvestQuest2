    ...
    async (req, res, next) => {
      try {
        ...
        await storage.db.transaction(async (trx) => {
          ...
          if (inputItem.quantity === requiredQuantity) {
            await trx('storage_items').where('id', inputItem.id).del();
          } else {
          ...
          if (existingOutput) {
            await trx('storage_items')
              ...
              .update({ quantity: existingOutput.quantity + outputQuantity });
          } else {
            await trx('storage_items').insert({
        ...
        try {
          ...
          await questService.updateQuestProgress(playerId, 'craft', {
            resourceId: process.output.resourceId,
            quantity: process.output.quantity * quantity
          });
        } catch (questError) {
